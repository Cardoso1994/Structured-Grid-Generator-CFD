...
X = np.genfromtxt('/home/desarrollo/Documents/Tesis_base/Potencial/potential_test/X.csv', delimiter=',')
Y = np.genfromtxt('/home/desarrollo/Documents/Tesis_base/Potencial/potential_test/Y.csv', delimiter=',')
phi_es = np.genfromtxt('/home/desarrollo/Documents/Tesis_base/Potencial/potential_test/phi.csv', delimiter=',')
C_es = np.genfromtxt('/home/desarrollo/Documents/Tesis_base/Potencial/potential_test/C.csv', delimiter=',')
theta_es = np.genfromtxt('/home/desarrollo/Documents/Tesis_base/Potencial/potential_test/theta.csv', delimiter=',')

plt.figure('potential')
plt.contour(mallaNACA.X, mallaNACA.Y, phi, 75)
plt.contour(X, Y, phi_es, 75)
...
X, mallaNACA.X = mallaNACA.X, np.flip(X)
Y, mallaNACA.Y = mallaNACA.Y, np.flip(Y)
(u_es, v_es) = velocity(alfa, C_es, mach_inf, theta_es, mallaNACA, phi_es, v_inf)
mallaNACA.X, X = X, np.flip(mallaNACA.X)
mallaNACA.Y, Y = Y, np.flip(mallaNACA.Y)

plt.figure('velocity')
plt.quiver(mallaNACA.X[:, 1:-1], mallaNACA.Y[:, 1:-1], u[:, 1:-1], v[:, 1:-1], scale=50, scale_units='x')
# plt.quiver(X[:, 1:-1], Y[:, 1:-1], u_es[:, 1:-1], v_es[:, 1:-1], scale=50, scale_units='x')
plt.plot(mallaNACA.X[:, 0], mallaNACA.Y[:, 0], 'k')
plt.plot(mallaNACA.X[:, -1], mallaNACA.Y[:, -1], 'k')
plt.axis('equal')
plt.draw()
