...
\usepackage[spanish,es-tabla, es-nodecimaldot]{babel} %es-tabla para nombrar a
                                                % las tablas en vez de cuadros
...
\addto{\captionsspanish}{\renewcommand*{\listfigurename}{Índice de Figuras}} %cambia el nombre del indice de figuras
...
% By default bibliography chapter is called Bibliografía, with this it changes
...
\begin{titlepage}
    \begin{center}
        % \begin{figure}[htbp!]
        %     \centering
            \includegraphics[keepaspectratio, width=77mm]{./img/ipn_logo}
        % \end{figure}
        \vspace*{2mm}
        \rule{\textwidth}{1.5pt}\\
        \vspace*{2mm}
        ...
        \vspace{2mm}
        \vspace{3mm}
        ASESOR: M. {\scriptsize EN} C. RAFAEL MEDINA NOGUERÓN
        \rule{\textwidth}{1.5pt}\\
    \end{center}
    \vspace{20mm}
    Ciudad de México \hfill 2020
\end{titlepage}
%
%
%
...
%
%
\includepdf[pages={1}, openright]{caratula_crop.pdf}
%
%
%
%
...
%
%
\includepdf[pages={3}, openright]{derechos_tesis.pdf}
%
%
%
%
...
\chapter*{Abstract}
The following work presents the source code development for the generation of
structured meshes with typologies O and C, for the computational analysis of
wing sections, also known as airfoils, with high lift devices such as external
airfoil flaps, this is achieved by solving a system of elliptical partial
differential equations. Computational simulations are carried on for both
potential flow and incompressible and steady flow, in order to prove the
reliability of the generated meshes.
...
\paragraph*{}
A mis hermanas, Ericka y Daniela, por su compañía y apoyo, por sus cuidados y
el ejemplo de superación y tenacidad que han sido durante toda mi vida.

\paragraph*{}
A Fiorella, por el amor, la confidencia y el apoyo a lo largo de estos
años. Gracias también por la paciencia y las enseñanzas que me han hecho abrir
mi mente y mi percepción del mundo y de la vida.

\paragraph*{}
A mis sobrinos, Leonardo, Emilio, Ivanna y Diego, por ser un constante
recordatorio de lo simple y alegre que puede ser la vida, por la
curiosidad y el entusiasmo que emanan.
...
\paragraph*{}
Al Instituto Politécnico Nacional, así como a la ESIME Ticomán, por los
recursos y la preparación que proporcionan al cuerpo estudiantil para nuestro
...
% indices
    ...
    \nomenclature[01]{DFC}{Dinámica de Fluidos Computacional}
    \nomenclature[02]{EDP}{Ecuaciones Diferenciales Parciales}

    \nomenclature[03]{$i$}{Indice i de malla estructurada}
    \nomenclature[04]{$j$}{Indice j de malla estructurada}
    \nomenclature[05]{$k$}{Indice k de malla estructurada}

    \nomenclature[06]{$x$}{Eje de las ordenadas en el dominio físico}
    \nomenclature[07]{$y$}{Eje de las abscisas en el dominio físico}
    \nomenclature[08]{$\xi$}{Eje de las ordenadas en el dominio computacional}
    \nomenclature[09]{$\eta$}{Eje de las abscisas en el dominio computacional}

    \nomenclature[10]{$f$}{Función de una propiedad del flujo}

    \nomenclature[11]{$\pdv{f}{x}$}{Derivada parcial de la propiedad del flujo
                    $f$ con respecto al eje $x$}
    \nomenclature[12]{$\pdv[2]{f}{x}$}{Segunda derivada parcial de la propiedad
                    del flujo $f$ con respecto al eje $x$}
    \nomenclature[13]{$\pdv[3]{f}{x}$}{Tercera derivada parcial de la propiedad
                    del flujo $f$ con respecto al eje $x$}
    \nomenclature[14]{$\pdv[n]{f}{x}$}{N derivada parcial de la propiedad del
                    flujo $f$ con respecto al eje $x$}
    \nomenclature[15]{$\pdv{f}{y}$}{Derivada parcial de la propiedad del flujo
                    $f$ con respecto al eje $y$}
    \nomenclature[16]{$\pdv[2]{f}{y}$}{Segunda derivada parcial de la propiedad
                    del flujo $f$ con respecto al eje $y$}
    \nomenclature[17]{$\pdv[3]{f}{y}$}{Tercera derivada parcial de la propiedad
                    del flujo $f$ con respecto al eje $y$}
    \nomenclature[18]{$\pdv[n]{f}{y}$}{N derivada parcial de la propiedad del
                    flujo $f$ con respecto al eje $y$}
    \nomenclature[19]{$\pdv{f}{x}{y}$}{Derivada parcial mixta de la propiedad
                    del flujo $f$ con respecto a los ejes $x$ y $y$}

    \nomenclature[20]{$f_{i, j}$}{Aproximación del valor de la propiedad $f$
                    del flujo en un punto $i, j$ de la malla}
    \nomenclature[21]{$\Delta x$}{Intervalo entre puntos en dirección del eje
                    $x$}
    \nomenclature[22]{$\Delta y$}{Intervalo entre puntos en dirección del eje
                    $y$}
    \nomenclature[23]{$O()$}{Error de truncamiento de la serie de Taylor}

    \nomenclature[24]{$L(x)$}{Polinomio de Lagrange en función de $x$}
    \nomenclature[25]{$L$}{Base polinómica de Lagrange}
    \nomenclature[26]{$H(x)$}{Polinomio de Hermite en función de $x$}
    \nomenclature[27]{$x_0$}{Coordenada x de un punto en la frontera interna de
                    la malla}
    \nomenclature[28]{$y_0$}{Coordenada x de un punto en la frontera interna de
                    la malla}
    \nomenclature[29]{$x_1$}{Coordenada x de un punto en la frontera externa de
                    la malla}
    \nomenclature[30]{$y_1$}{Coordenada x de un punto en la frontera externa de
                    la malla}
    \nomenclature[31]{$r$}{Punto arbitrario de la malla expresado en
                    coordenadas del dominio computacional $\xi, \eta$}
    \nomenclature[32]{$P_\xi$}{Proyector del dominio computacional al dominio
                    físico mapeando las coordenadas en el eje $\xi$}
    \nomenclature[33]{$P_\eta$}{Proyector del dominio computacional al dominio
                    físico mapeando las coordenadas en el eje $\eta$}

    \nomenclature[34]{$\xi_{xx}$}{Segunda derivada parcial de $\xi$ con
                    respecto a $x$}
    \nomenclature[35]{$\xi_{yy}$}{Segunda derivada parcial de $\xi$ con
                    respecto a $y$}
    \nomenclature[36]{$\eta_{xx}$}{Segunda derivada parcial de $\eta$ con
                    respecto a $x$}
    \nomenclature[37]{$\eta_{yy}$}{Segunda derivada parcial de $\eta$ con
                    respecto a $y$}
    \nomenclature[38]{$x_{\xi\xi}$}{Segunda derivada parcial de $x$ con
                    respecto a $\xi$}
    \nomenclature[39]{$x_{\xi\eta}$}{Derivada parcial mixta de $x$ con respecto
                    a $\xi$ y $\eta$}
    \nomenclature[40]{$x_{\eta\eta}$}{Segunda derivada parcial de $x$ con
                    respecto a $\eta$}
    \nomenclature[41]{$y_{\xi\xi}$}{Segunda derivada parcial de $y$ con
                    respecto a $\xi$}
    \nomenclature[42]{$y_{\xi\eta}$}{Derivada parcial mixta de $y$ con respecto
                    a $\xi$ y $\eta$}
    \nomenclature[43]{$y_{\eta\eta}$}{Segunda derivada parcial de $y$ con
                    respecto a $\eta$}

    \nomenclature[44]{$P$}{Función de forzado en dirección del eje $\xi$ para
                    la solución de la ecuación de Poisson}
    \nomenclature[45]{$Q$}{Función de forzado en dirección del eje $\eta$ para
                    la solución de la ecuación de Poisson}
    \nomenclature[46]{$J$}{Jacobiano de la transformación}
    \nomenclature[47]{$I$}{Inverso del Jacobiano de la transformación}

    \nomenclature[48]{$X$}{Coordenada en el eje $x$ del dominio computacional
                    de un punto perteneciente a la malla}
    \nomenclature[49]{$Y$}{Coordenada en el eje $y$ del dominio computacional
                    de un punto perteneciente a la malla}

    \nomenclature[50]{$\vec{v}$}{Vector de velocidad del flujo en el dominio
                    físico}
    \nomenclature[51]{$\vec{\nabla}$}{Operador vectorial nabla}
    \nomenclature[52]{$\phi$}{Función de flujo potencial}
    \nomenclature[53]{$H$}{Entalpía}
    \nomenclature[54]{$H_0$}{Entalpía de estancamiento}
    \nomenclature[55]{$\rho$}{Densidad}
    \nomenclature[56]{$\rho_0$}{Densidad de estancamiento}
    \nomenclature[57]{$\gamma$}{Relación de calores específicos de un gas}
    \nomenclature[58]{$U$}{Componente de la velocidad en el dominio
                    computacional en dirección del eje $\xi$}
    \nomenclature[59]{$V$}{Componente de la velocidad en el dominio
                    computacional en dirección del eje $\eta$}
    \nomenclature[60]{$\phi_{x}$}{Primer derivada parcial de la función de
                    flujo potencial con respecto a $x$}
    \nomenclature[61]{$\phi_{y}$}{Primer derivada parcial de la función de
                    flujo potencial con respecto a $y$}
    \nomenclature[62]{$\phi_{\xi}$}{Primer derivada parcial de la función de
                    flujo potencial con respecto a $\xi$}
    \nomenclature[63]{$\phi_{\eta}$}{Primer derivada parcial de la función de
                    flujo potencial con respecto a $\eta$}
    \nomenclature[64]{$\vec{V}_{\infty}$}{Vector de velocidad del flujo en
                    corriente libre}
    \nomenclature[65]{$M$}{Número de Mach}
    \nomenclature[66]{$\Gamma$}{Circulación}
    \nomenclature[67]{$\alpha$}{Ángulo de ataque del perfil}
    \nomenclature[68]{$\delta$}{Ángulo de deflexión del flap}
    ...
    desarrollo de las capacidades computacionales, ya que las simulaciones
    presentan ciertas ventajas frente al enfoque experimental en términos de
    ...
    algún software \textit{``solver''}, para poder llevar a cabo el
    análisis de flujo deseado, en este caso se exportará al formato del
    programa SU2.
...
\pagenumbering{arabic}
    ...
    tecnológicas de los ordenadores, así como el fácil acceso a los mismos,
    ...
    compartir y modificar el código fuente del software.
    ...
    OpenFOAM y SU2 con los cuales la interacción con el programa se da mediante
    la línea de comandos. También proporciona servicios en la nube. A pesar de
    ser software de código abierto, es de paga.\cite{helyx}
...
\chapter{Discretización Espacial}\label{chap:discretizacion-espacial}
    ...
    cuadriláteros, mientras que en un caso tridimensional la malla es
    ...
    \item Estructuradas: los nodos de la malla están alineados entre sí y
      ...
      tridimensional se forma por hexahédros.
            Figura~\ref{fig:malla_estructurada__}.
    \item No estructuradas: los nodos no tienen un orden particular, esto
      ...
      llevar a cabo una forma diferente de identificación.
            Figura~\ref{fig:malla_no_estructurada__}
    \end{itemize}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.45\textwidth}
            ...
            \label{fig:malla_estructurada__}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
            ...
            \label{fig:malla_no_estructurada__}
        \end{subfigure}
    ...
    \paragraph*{}
    El método de diferencias finitas fue de los primeros métodos numéricos
    ...
    \paragraph*{}
    ``El uso del método de diferencias finitas en la DFC tiene como propósito
    el reemplazar las derivadas parciales que aparecen en las ecuaciones que
    gobiernan el flujo, por coeficientes diferenciales que dan como resultado
    un sistema de ecuaciones algebráicas, el cual se resuelve para obtener las
    ...
    \begin{figure}[htbp!]
        \centering
        \includegraphics[keepaspectratio, width=95mm]{./img/malla-DF1}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=\textwidth]{./img/grafica-forward}
            \caption{Aproximación forward}
            \label{fig:grafica_forward}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
...
\paragraph{}
    ...
    \begin{equation}
    \left( \pdv{f}{x}{y} \right)_{i,j} =
        \frac{\left( \pdv{f}{y} \right)_{i+1, j}
        - \left( \pdv{f}{y} \right)_{i-1,j}}{2 \Delta x}
    \end{equation}
    y desarrollando una diferencia central para la derivada parcial de $f$ con
    respecto a $y$ tenemos:
    \begin{equation}
    \left( \pdv{f}{x}{y} \right)_{i,j} = \frac{1}{2 \Delta x}
        \left[ \frac{f_{i+1, j+1} - f_{i+1, j-1}}{2 \Delta y}
            - \frac{f_{i-1, j+1} - f_{i-1, j-1}}{2 \Delta y} \right]
    \end{equation}
    \begin{equation}
        \left( \pdv{f}{x}{y} \right)_{i,j} =
            \frac{1}{4 \Delta x \Delta y} \left( f_{i+1, j+1}
                + f_{i-1, j-1} - f_{i+1, j-1} - f_{i-1, j+1} \right)
    \end{equation}

    \paragraph*{}
    La misma metodología se aplica para obtener las aproximacinoes por el método
    de diferencias finitas de una función respecto a un eje cualquiera, llámese
    eje $y$, $\xi$ o $\eta$.
    ...
    métodos de generación de mallas, ya que se busca trabajar con mallas para
    las cuales, en sus sistemas de coordenadas computacionales, pueda ser
    aplicado el método de diferencias finitas para la solución de problemas.
    %
    %
    %
    %
    ...
    medio continuo, discretizando las ecuaciones con respecto a la malla y por
    ...
    geometrías complejas, lo que hace que sea muy difícil, si no imposible,
    generar una malla en alguno de los sistemas de coordenadas antes
    ...
    se puede hacer uso de cualquier forma geométrica. Para este trabajo se
...
\section{Tipos de Mallas Estructuradas}
    \paragraph*{}
    Existen tres tipos base de mallas estructuradas conocidos como mallas
    C, H u O respectivamente. El nombre que dichos tipos de mallado reciben se
    debe a que su estructura asemeja a dichas letras (en mayúsculas) desde una
    vista de planta.
    ...
    interior el cual corresponde a la frontera interna del dominio físico.
    ...
    debe generar una malla solo lo suficientemente densa para que la
    ...
    un mallado adecuado para el análisis que se pretende realizar. Sin
    embargo, sí hay ciertas consideraciones a tomar en cuenta, tales como la
...
\section{Generación de mallas mediante métodos algebráicos}
    ...
    $\eta$. Es decir, para interpolar en la dirrección $\xi$, se deben
    seleccionar puntos en las frontera tanto interna como externa, que
    correspondan a la misma línea $\eta = constante$ y a lo largo de la cual se
    llevará a cabo la interpolación, haciendo la variación en la coordenada
    $\xi$, interpolando así, línea por línea hasta terminarse de generar la
    malla. El mismo procedimiento se puede llevar a cabo interpolando en la
    dirección de $\eta$.
    ...
    \paragraph*{}
    Las figuras~\ref{fig:malla-inter-completa} y~\ref{fig:malla-inter-cerca}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=0.99\textwidth]{./img/M-inter_pol}
            \caption{Vista Completa}
            \label{fig:malla-inter-completa}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio, 
    ...
    interpolación simultánea, en el espacio computacional, en ambas
    ...
    del dominio físico, proprocionados por el usuario, para valores dados de
    ...
    frontera externa y una frontera interna, que se pretende analizar.
...
\section{Generación de mallas mediante EDP}
    ...
    \begin{subequations}
        ...
        \label{ec-poisson}
    \end{subequations}\\
    si de nuevo, esta ecuación es transformada, volviendo las variables $x$ y
    $y$ las variables dependientes, queda expresada como
    \begin{subequations}
        \begin{equation}
            \alpha x_{\xi \xi} - 2 \beta x_{\xi \eta}
    ...
    los cálculos requeridos, y que además también es sustancialmente menor
    que la requerida por un esquema elíptico.
    ...
    versatilidad en la generación de la misma.
    ...
    \begin{align*}
        &\begin{aligned}
            &&&&&&&&&&&&&&&&&&A = \frac{2\alpha}{F_{i - 1} (F_i + F_{i - 1}) }
                &&& ; &&& C =& \frac{2\alpha}{F_i (F_i + F_{i - 1}) }
        \end{aligned}\\ \\
        &\begin{aligned}
            &&&&&&&&&&&&&&&B = \frac{-2 \alpha}{F_i + F_{i - 1}}
                \left( \frac{1}{F_i} + \frac{1}{F_{i - 1}} \right)
                - \frac{2\gamma}{G_j + g_{j - 1}} \left( \frac{1}{G_j}
                + \frac{1}{g_{j - 1}} \right)
        \end{aligned}\\ \\
        &\begin{aligned}
            D_x = -\beta \frac{ XO_{i+1, j+1} - XO_{i-1, j+1} - x_{i+1, j-1}
                + x_{i-1, j-1} }{ (F_i + F_{i - 1}) (G_j + g_{j - 1}) }
                - \frac{2 \gamma}{G_j + g_{j - 1}}
                \left[ \frac{x_{i, j - 1}}{g_{j-1}}
                + \frac{XO_{i, j + 1}}{G_j} \right]
        \end{aligned} \\ \\
        &\begin{aligned}
            D_y = -\beta \frac{ YO_{i+1, j+1} - YO_{i-1, j+1} - y_{i+1, j-1}
                + y_{i-1, j-1} }{ (F_i + F_{i - 1}) (G_j + g_{j - 1}) }
                - \frac{2 \gamma}{G_j + g_{j - 1}}
                \left[ \frac{y_{i, j - 1}}{g_{j-1}}
                + \frac{YO_{i, j + 1}}{G_j} \right]
        \end{aligned}
    \end{align*}\\
    ...
    \ref{fig:parabolic_modified} una recta $AA$ que es perpendicular a la
    ...
    \begin{figure}[htbp!]
        \centering
        \includegraphics[keepaspectratio, width=110mm]{./img/aspect_ansys.png}
...
\chapter{Análisis de Flujo Potencial}\label{chap:potential}
    ...
    \paragraph*{}
        ...
        $\Gamma$, sobre éstas se deben especificar las correspondientes
        condiciones de frontera que dotarán de sentido físico a la simulación,
        es decir, para este caso se deben especificar condiciones de frontera
        que coincidan con la teoría de flujo potencial.
    ...
    \paragraph*{}
        ...
        llegar al borde de salida, a partir de que se logra este fenómeno se
    ...
    \paragraph*{}
        ...
        \begin{figure}[htbp!]
            \centering
            \includegraphics[keepaspectratio, width=115mm]{./img/kutta_condition}
    ...
    \paragraph*{}
        ...
        \begin{figure}[htbp!]
            \centering
            \includegraphics[keepaspectratio, 
                width=115mm]{./img/circulacion_tiempo}
            \caption[Flujo alrededor de perfil momentos después de iniciado el
            flujo]{Flujo alrededor de perfil momentos después de inciado el
            flujo~\cite{anderson_fundamentals}}
    ...
    \paragraph*{}
        ...
        discretización mediante los métodos de volúmenes y elementos finitos.
    ...
    \paragraph*{}
        ...
        baja densidad de celdas, a excepción de ciertas zonas donde se generan
        grandes gradientes como lo son esquinas, bordes de ataque y de salida
    ...
    \paragraph*{}
        Si en la ecuación~\ref{eq_potencial_0} se eliminan los términos
...
\chapter{Desarrollo Práctico}
    ...
    \paragraph*{}
        ...
        coeficientes de presión ($c_p$) y de sustentación ($c_l$).
    ...
    \begin{figure}[htbp!]
        ...
        \caption{Diagrama de flujo para el análisis de flujo potencial}
...
\chapter{Resultados}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[b]{0.40\textwidth}
            \includegraphics[keepaspectratio,
                width=0.95\textwidth]{./img/naca4415_50_far}
            \caption{$50\si{\degree}$ Vista del perfil}
            \label{fig:naca4415_50_far}
        \end{subfigure}
        \hfill
        \begin{subfigure}[b]{0.40\textwidth}
            \includegraphics[keepaspectratio,
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=0.99\textwidth]{./img/naca4415_flap_far}
            \caption{Vista Completa}\label{fig:naca4415_flap_far}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=0.99\textwidth]{./img/naca4415_flap_close}
            \caption{Acercamiento a perfil}\label{fig:naca4415_flap_close}
        \end{subfigure}
        \caption[Malla tipo O con perfil y flap NACA 4415]{Malla tipo O con
            perfil y flap NACA 4415, ambos con ángulo de ataque
            $\alpha = 0\si{\degree}$}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[t]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=\textwidth]{./img/naca4415_flap_50}
            \caption{Flap a $50\si{\degree}$}
            \label{fig:naca4415_flap_50_both}
        \end{subfigure}
        \hfill
        \begin{subfigure}[t]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=0.97\textwidth]{./img/naca4415_flap_50_airfoil}
            \caption{Acercamiento a borde de salida de perfil y borde de
                ataque de flap}
            \label{fig:naca4415_flap_50_airfoil}
        \end{subfigure}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=0.99\textwidth]{./img/naca4415_c_flap_20}
            \caption{Acercamiento a perfil y flap}
            \label{fig:naca4415_c_flap_20}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
                width=0.99\textwidth]{./img/naca4415_c_flap_20_both}
            \caption{Acercamiento a borde de salida de perfil}
            \label{fig:naca4415_c_flap_20_both}
        \end{subfigure}
        \begin{subfigure}[c]{0.45\textwidth}
            \includegraphics[keepaspectratio,
    ...
    bordes de salida debido a que las líneas continúan con su recorrido de
    ...
    realizados son $M$ de $M=73$ a $M=91$, $N$ de $N=45$ a $N=65$ y $aa_{m}$ de
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.48\textwidth}
            \includegraphics[keepaspectratio, width=0.99\textwidth]
                {./img/naca4415_c_aspect_far}
            \caption{Vista Completa}
            \label{fig:naca4415_c_aspect_far}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.48\textwidth}
            \includegraphics[keepaspectratio, width=0.99\textwidth]
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.48\textwidth}
            ...
            \caption{Acercamiento a perfil}
            \label{fig:potential_flow_potential__}
        \end{subfigure}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.48\textwidth}
            ...
            \caption{$\alpha = -4\si{\degree}$}
            \label{fig:potential_flow_stream_-4}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.48\textwidth}
            \includegraphics[keepaspectratio, width=0.99\textwidth]
    ...
    \paragraph*{}
    A su vez, las figuras~\ref{fig:potential_flow_2412_cp__0}
    y~\ref{fig:potential_flow_2412_cp__1} presentan el campo de presiones que
    se genera alrededor del perfil para diferentes ángulos de ataque. Por otro
    lado las figuras~\ref{fig:potential_flow_2412_stream__0}
    y~\ref{fig:potential_flow_2412_stream__1} muestran las líneas de corriente
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.48\textwidth}
            \includegraphics[keepaspectratio, width=0.99\textwidth]
                {./img/potential_flow_2412_potential_far}
            \caption{Vista Completa}
            \label{fig:potential_flow_2412_potential_far}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.48\textwidth}
            \includegraphics[keepaspectratio, width=0.99\textwidth]
                {./img/potential_flow_2412_potential_close}
            \caption{Acercamiento a perfil}
            \label{fig:potential_flow_2412_potential__}
        \end{subfigure}
        \caption{Líneas equipotenciales para $\alpha = 0\si{\degree}$
            del análisis de flujo potencial de perfil NACA 2412}
    ...
    \begin{figure}[htbp!]
        ...
        \begin{subfigure}[c]{0.48\textwidth}
            ...
            \caption{$\alpha = -4\si{\degree}$}
            \label{fig:potential_flow_2412_stream_-4}
        \end{subfigure}
        \hfill
        \begin{subfigure}[c]{0.48\textwidth}
            \includegraphics[keepaspectratio, width=0.99\textwidth]
    ...
    es un esfuerzo de la comunidad para estandarizar la representación de la
    información para la solucion de simulaciones para \@DFC.
    ...
    \paragraph*{}
    Por último se muestra en la figura~\ref{fig:cp_comparison_10} una
    comparación del valor del coeficiente de presión para el perfil NACA 0012 a
    un ángulo de ataque $\alpha = 10\si{\degree}$ para tres casos diferentes,
    dos de ellos son los resultados presentados en este trabajo, tanto malla
    tipo C como tipo O, además de éstos se presentan los resultados de la
    simulación del mismo problema utilizando una malla estructurada tipo C
    creada en el Centro de Investigación Langley de la
    NASA~\cite{malla_C_NASA}, dicha malla es proporcionada como parte del
    tutorial del programa SU2.
...
\paragraph*{}
    ...
    de puntos que describa el perfil deseado, a través de un archivo de valores
    separados por comas. Además de esto se desarrollaron los módulos necesarios
    para la generación de mallas, tanto tipo C como tipo O, mediante diversos
...
\paragraph*{}
    ...
    mejoras sustanciales en el desempeño de la aplicación, entre ellas se
    pueden destacar
    \begin{itemize}
...
\paragraph*{}
    ...
    desarrollando. La DFC es un campo cuyas características implican una
...
\paragraph*{}
    ...
    particular atención en la estabilidad numérica de los métodos. Tambien se
...
\lstinputlisting[language=Python, breaklines=true]{./code/mesh_C_poisson.py}

\section{Generación de Malla O con uso de librería numba}
\lstinputlisting[language=Python, breaklines=true]{./code/mesh_O_poisson.py}

\section{Conversión de mallas a formato SU2}
\lstinputlisting[language=Python, breaklines=true]{./code/mesh_su2.py}

\section{Archivo main. Ejemplo de ejecución}
\lstinputlisting[language=Python, breaklines=true]{./code/main_example.py}
%
%
%
%
...
\chapter{Métodos Numéricos Iterativos}\label{appIter}
    \paragraph*{}
        ...
        algebráicas lineales simultáneas: métodos directos y métodos
        iterativos.

    \paragraph*{}
        Dentro de los métodos directos podemos encontrar los métodos de Cramer
        y de Gauss. La principal desventaja que presentan estos metodos es el
        gran número de opraciones aritméticas necesarias para obtener una
        solución del sistema. Desde el punto de vista computacional otras
        desventajas que presentan son el uso de memoria y cierta dificultad para
        ser programados. Los métodos iterativos son simples y fáciles de
        programar, por lo que se presentan como una mejor herramienta para
        llevar a cabo la solución del sistema. La idea de estos métodos, tal y
        como su nombre lo indica, es obtener la solución mediante un proceso
        iterativo, por lo general se asume una primera solución y con dichos
        valores propuestos se calculan nuevos valores de las incógnitas, con
        base en lo nuevos valores calculados se obtienen nuevos valores. Este
        proceso se repite hasta satisfacer el criterio de convergencia
        establecido.
...
\chapter{Solución de Sistemas de Bloque Tridiagonal}\label{appTridiagonal}
    \paragraph*{}
        En el presente apéndice se lleva a cabo la solución de un sistema de
        bloque tridiagonal, como los obtenidos en los métodos de generación de
        mallas estructuradas tanto por EDPs hiperbólicas, como parabólicas, el
        método es desarrollado por Hoffman\cite{hoffmann2000computational}.
    \paragraph*{}
        ...
        \begin{equation}
            ...
            \label{hyper-sol-1}
        \end{equation}\\
        donde $\Delta Q$ y $R$ son vectores con $m$ componentes, y a su vez, $S$
