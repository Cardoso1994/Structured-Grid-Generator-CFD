...
def potential_flow_o(d0, H0, gamma, mach_inf, v_inf, alfa, mesh):
    ...
    g11V[:, :-1]    = 0.5 * (g11[:, :-1] + g11[:, 1:])
    g12V[:, :-1]    = 0.5 * (g12[:, :-1] + g12[:, 1:])
    g22V[:, :-1]    = 0.5 * (g22[:, :-1] + g22[:, 1:])
    JV[:, :-1]      = 0.5 * (J[:, :-1] + J[:, 1:])
    x_xiV[:, :-1]   = 0.5 * (x_xi[:, :-1] + x_xi[:, 1:])
    x_etaV[:, :-1]  = 0.5 * (x_eta[:, :-1] + x_eta[:, 1:])
    y_xiV[:, :-1]   = 0.5 * (y_xi[:, :-1] + y_xi[:, 1:])
    y_etaV[:, :-1]  = 0.5 * (y_eta[:, :-1] + y_eta[:, 1:])
    ...
    g11H[:-1, :]    = 0.5 * (g11[:-1, :] + g11[1:, :])
    g12H[:-1, :]    = 0.5 * (g12[:-1, :] + g12[1:, :])
    g22H[:-1, :]    = 0.5 * (g22[:-1, :] + g22[1:, :])
    JH[:-1, :]      = 0.5 * (J[:-1, :] + J[1:, :])
    x_xiH[:-1, :]   = 0.5 * (x_xi[:-1, :] + x_xi[1:, :])
    x_etaH[:-1, :]  = 0.5 * (x_eta[:-1, :] + x_eta[1:, :])
    y_xiH[:-1, :]   = 0.5 * (y_xi[:-1, :] + y_xi[1:, :])
    y_etaH[:-1, :]  = 0.5 * (y_eta[:-1, :] + y_eta[1:, :])
